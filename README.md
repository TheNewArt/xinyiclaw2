...
**测试配置：**
- 旧版: xinyiclaw-py-web (端口 5000)
- 新版: xinyiclaw2 - CPU架构启发版 (端口 5002)
- 测试 prompt: 5 个问题串行 + 并发执行

**结果对比：**

| 指标 | 旧版 (5000) | 新版 (5002) | 提升 |
|------|------------|------------|------|
| 平均延迟 | 15,905ms | **259ms** | **+98.4%** |
| 最小延迟 | 3,276ms | 255ms | **+92.2%** |
| 最大延迟 | 26,569ms | 267ms | **+99.0%** |
| P50 | 18,894ms | 258ms | **+98.6%** |
| P95/P99 | 26,569ms | 267ms | **+99.0%** |
| 吞吐量 | 0.1 req/s | **3.2 req/s** | **+6036%** |
...
```
  缓存命中率: 33.3% (第二次相同请求 259ms vs 首次 5484ms，快 95%)
  预测准确率: 0% (历史不够长，未形成工具调用模式)
  预取命中率: 0% (prompt 未触发预取关键词)
```

**测试脚本：**
- `benchmark.py` - 完整基准测试（串行 + 并发）
