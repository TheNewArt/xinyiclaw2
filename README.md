...
| 2026-04-03 | `Semaphore is bound to a different event loop` — Flask debug 模式重启导致 event loop 变化 | 改用延迟创建 `@property` 模式，首次使用时才创建 Semaphore | ✅ 已修复 |
...
| 指标 | 旧版 (5000) | 新版 (5002) | 提升 |
|------|------------|------------|------|
| 平均延迟 | 15,905ms | 10,783ms | **+32%** |
| 最大延迟 | 26,569ms | 26,500ms | **+0.3%** |
| P50 | 18,894ms | 15,000ms | **+21%** |

### 架构优化效果

| 功能 | 旧版 | 新版 |
|------|------|------|
| 缓存机制 | 无 | blake2b 完整 hash，相似问题不误命中 |
| 错误处理 | 无重试，直接返回错误 | 520/0 错误自动重试 3 次 |
| API 稳定性 | API 错误直接暴露 | 错误响应不缓存，优雅降级 |
| 指标监控 | 无 | MetricsCollector 全链路追踪 |

### 核心指标

| 指标 | 说明 | 实测值 |
|------|------|--------|
| 缓存命中率 | 重复问题直接返回 | **33.3%** (测试中) |
| 错误恢复率 | API 错误后自动重试 | **100%** |
| 零丢失 | 请求不丢失 | **100%** |
| 延迟优化 | 缓存命中 vs 首次调用 | **95%+** (259ms vs 5s) |

### 测试脚本

- `benchmark.py` - 完整基准测试（对比旧版）
- `demo_realtime.py` - 实时对比演示
- `test_metrics.py` - Metrics 功能专项测试
- `test_fair.py` - 公平测试（清空缓存后）
